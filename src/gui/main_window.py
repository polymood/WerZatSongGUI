...
class MainWindow(QMainWindow):
    """Main window with Home plus job tabs and live QSettings support."""
    ...
    def __init__(self) -> None:
        ...
        self.youtube_index = self.tabs.addTab(
            self.youtube_page, "YouTube Jobs (yt-dlp)"
        )
        self.fprint_index = self.tabs.addTab(
            self.fprint_page, "Fingerprint Jobs (audfprint)"
        )
        self.scan_index = self.tabs.addTab(
            self.scan_page, "Scan Jobs"
        )
        self.ffmpeg_index = self.tabs.addTab(
            self.ffmpeg_page, "FFmpeg Jobs"
        )

        self.tabs.setCurrentIndex(self.home_index)

        # 4) Prepare SettingsDialog (no translation)
    ...
    def _create_menu_bar(self):
        ...
        act_rescan = QAction("Rescan System Info", self)
        act_rescan.triggered.connect(self._rescan_system_info)
        tools_menu.addAction(act_rescan)

        # Settings menu
        settings_menu = menubar.addMenu("Settings")
        act_settings = QAction("App Settings", self)
        act_settings.triggered.connect(self._open_settings)
        act_edit_config = QAction("Open settings storage", self)
        ...
        act_feedback = QAction("Report a Bug / Feedback", self)
    ...
    def _show_about(self):
        ...
        QMessageBox.about(
            ...
            (
                ...
                "Powered by audfprint, ffmpeg, yt-dlp, PyQt, and open source contributors.<br>"
            )
        )
    ...
    def _rescan_system_info(self):
        ...
        QMessageBox.information(
            self,
            "Scan complete",
            "System info has been rescanned and saved to settings."
        )
